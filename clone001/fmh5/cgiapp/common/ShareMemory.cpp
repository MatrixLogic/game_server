...
bool CShareMemory::Create(const char *name, int size, int semid, unsigned vnum)
{
	if(name == NULL)
	{
		return false;
	...
	if(!m_lock.Create(lockName.c_str(),semid, vnum))
	{
		Close();
		return false;
	}
...
bool CShareMemory::Open(const char *name, int size, int semid, unsigned vnum)
{
	if(name == NULL)
	{
		return false;
	...
	if(!m_lock.CreateOrOpen(lockName.c_str(),semid, vnum))
	{
...
bool CShareMemory::CreateOrOpen(const char *name, int size, int semid, unsigned vnum)
{
	if(!Open(name, size,semid, vnum))
	{
		if(!Create(name, size,semid, vnum))
		{
			return false;
		}
	}
...
bool CShareMemory::Lock(int vnum)
{
	if(m_lock.Lock(vnum))
	{
...
bool CShareMemory::Unlock(int vnum)
{
	if(m_lock.Unlock(vnum))
	{
		((ShareMemoryHead *)m_pAddress)->Lock = 0;
		return true;
	}
