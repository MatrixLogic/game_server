...
bool CSocket::GetLocalAddress(sockaddr *address, int *addressLength)
{
	socklen_t lengthTemp = (socklen_t)*addressLength;
	if(getsockname(m_socket, address, &lengthTemp) != 0)
	{
		return false;
	}
	*addressLength = (int)lengthTemp;
...
bool CSocket::GetRemoteAddress(sockaddr *address, int *addressLength)
{
	socklen_t lengthTemp = (socklen_t)*addressLength;
	if(getpeername(m_socket, address, &lengthTemp) != 0)
	{
		return false;
	}
	*addressLength = (int)lengthTemp;
