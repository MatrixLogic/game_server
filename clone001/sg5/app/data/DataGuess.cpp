...
CGuessData::CGuessData(){
	m_init = false;
}


int CGuessData::Init(const string &path,int type, semdat sem){
	...
	char fname[300] = {0};
	snprintf(fname,sizeof(fname),"%s.%d",path.c_str(),type);
	...
	if(!m_sh.CreateOrOpen(fname,sizeof(GuessData), SEM_ID(sem,semgroup,semserver))){
...
int CGuessData::Apply(unsigned uid,unsigned gid,unsigned coins){
	GuessData *pData = (GuessData *)m_sh.GetAddress();
	if(pData == NULL){
...
int CGuessData::View(unsigned uid,vector<GuessViewData>& dataVec){
	GuessData *pData = (GuessData *)m_sh.GetAddress();
	if(pData == NULL){
		error_log("get GuessData address failed");
		return R_ERR_DB;
...
int CGuessData::SetInitValues(const vector<GuessViewData>& dataVec){
	GuessData *pData = (GuessData *)m_sh.GetAddress();
	if(pData == NULL){
		error_log("get GuessData address failed");
		return R_ERR_DB;
...
int CGuessData::GetSupportCount(unsigned gid,unsigned& count){
	GuessData *pData = (GuessData *)m_sh.GetAddress();
	if(pData == NULL){
		error_log("get GuessData address failed");
		return R_ERR_DB;
